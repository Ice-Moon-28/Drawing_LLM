...
"""
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        参数:
            ...
            attention_mask: 注意力 mask，形状为 [batch_size, seq_len]
            labels: 如果是训练，则传入 labels 以计算 loss
        返回:
            如果提供 labels，则返回 loss；否则返回 logits
        """
        outputs = self.model(
            input_ids=input_ids,
        ...
        if labels is not None:
            # 交叉熵损失函数忽略 padding token
            loss_fct = torch.nn.CrossEntropyLoss(ignore_index=-100)
            # 调整维度为 [batch_size * seq_len, vocab_size] 与 [batch_size * seq_len]
